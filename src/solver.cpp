...
using namespace std;
using Probabilities = vector<double>;
    ...
    for (int hi = 0; hi < H; ++hi)
    {
        ...
        for (const auto &trip : plan.trips)
        {
            ...
            for (const auto &dp : trip.drops)
            {
                ...
                otherDelivered[v.id - 1] += useOther;
                totalDrops++;
            }
            // return home
...
// Generate a tabu-free neighbor (move and candidate) from current solution
// Generate a tabu-free neighbor (move and candidate) from current solution, using move probabilities
static void generateNeighbor(const ProblemData &problem,
                             ...
                             const Probabilities &probs)
{
    ...
    do
    {
        ...
        if (r < 0.20)
            ...
            mv.type = 5;
        // --- neighbor generation code for types 0-5 ---
        if (mv.type == 0)
        {
...
// Forward declarations of SA helpers
static void generateNeighbor(const ProblemData &, const Solution &, Solution &, Move &, deque<Move> &, mt19937_64 &, uniform_real_distribution<double> &, const Probabilities &);
static bool acceptCandidate(double, double, double, mt19937_64 &, uniform_real_distribution<double> &);
static double defaultCooling(double);

// SA helper definitions
static bool acceptCandidate(double candObj, double curObj, double T, mt19937_64 &rng, uniform_real_distribution<double> &unif)
{
    if (candObj > curObj)
        return true;
    return unif(rng) < exp((candObj - curObj) / T);
}

static double defaultCooling(double T)
{
    return T * 0.95;
}

Solution greedy_solution(const ProblemData &problem)
{
    Solution greedy;
    int V = problem.villages.size();
        ...
        while (true)
        {
            ...
            while (true)
            {
                ...
                for (int i = 0; i < V; ++i)
                {
                    ...
                    long loadP = min((long)floor(remW / pkg[1].weight), (long)floor(remFood[i]));
                    double wP = loadP * pkg[1].weight;
                    long loadD = min((long)floor((remW - wP) / pkg[0].weight), (long)floor(remFood[i] - loadP));
                    double wD = loadD * pkg[0].weight;
                    long loadO = min((long)floor((remW - wP - wD) / pkg[2].weight), (long)floor(remOther[i]));
                ...
                double remW = heli.weight_capacity - tripWeight;
                const auto &pkg = problem.packages;
                long loadP = min((long)floor(remW / pkg[1].weight), (long)floor(remFood[bestV]));
                double wP = loadP * pkg[1].weight;
                long loadD = min((long)floor((remW - wP) / pkg[0].weight), (long)floor(remFood[bestV] - loadP));
                double wD = loadD * pkg[0].weight;
                long loadO = min((long)floor((remW - wP - wD) / pkg[2].weight), (long)floor(remOther[bestV]));
                // update trip totals
            ...
            if (!added)
                break;
            // return home distance and check profitability
            double back = euclid(curr, home);
            tripDist += back;
            // compute delivered value of this trip
            double tripValue = 0.0;
            for (auto &dp : trip.drops)
            {
                tripValue += dp.perishable_food * problem.packages[1].value + dp.dry_food * problem.packages[0].value + dp.other_supplies * problem.packages[2].value;
            }
            double tripCost = heli.fixed_cost + heli.alpha * tripDist;
            if (tripValue <= tripCost + 1e-9)
            {
                // unprofitable trip, stop further trips
                break;
            }
            usedDist += tripDist;
...
// Run SA on an initial solution for given time and move probabilities
static Solution runSA(const ProblemData &problem,
                      const Solution &init,
                      double timeBudget,
                      const Probabilities &probs)
{
    Solution current = init, best = init;
    ...
    deque<Move> tabu;
    while (chrono::duration<double>(chrono::steady_clock::now() - start).count() < timeBudget)
    {
        // status print omitted for brevity
        Solution cand;
        Move mv{0, 0, 0, 0, 0, 0, 0};
        generateNeighbor(problem, current, cand, mv, tabu, rng, unif, probs);
        ...
        if (candObj > curObj || unif(rng) < exp((candObj - curObj) / T))
        {
            current = cand;
            curObj = candObj;
            tabu.push_back(mv);
...
// Main solve: dual‐seed strategy
Solution solve(const ProblemData &problem)
{
    int C = problem.cities.size();
    int V = problem.villages.size();
    double totalTime = max(0.0, problem.time_limit_minutes * 60.0 - 5.0);

    // multi‐stop greedy seed
    auto choice = greedy_solution(problem);

    // final SA on remaining time with standard probs
    auto best = runSA(problem, choice, totalTime,
                      Probabilities{0.20, 0.30, 0.10, 0.25, 0.05, 0.10});
    return best;
}